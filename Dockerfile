...
RUN python3 -m virtualenv --download "${VIRTUAL_ENV}"
...
ARG DEFAULT_MODULES
RUN python3 -m pip install --only-binary=:all: --find-links "/${IMPORTS_DIR}/"  ${DEFAULT_MODULES} \
	|| python3 -m pip install --find-links "/${IMPORTS_DIR}/" ${DEFAULT_MODULES}
...
RUN echo "Building ${MODULE_NAME}==${MODULE_VERSION}.." \
	...
	&& [ ! -z "${WHEEL_BUILT_IN_SCRIPT+set}" ] \
		|| python3 -m pip wheel --find-links "/${IMPORTS_DIR}/" -w "${WHEELS_DIR}" "${MODULE_NAME}==${MODULE_VERSION}"
